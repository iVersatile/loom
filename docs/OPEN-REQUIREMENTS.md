...
| **0017** Writer remote-trust | Writer pushes branches; merge-to-main gated | 🟠 policy live, headline UNREALIZED — Writer still advisor-only push | none (no FR-PUSH — behavior unshipped) | **NOT credential-blocked** (the "036" framing was a misnomer — the gh fine-grained PAT is provisioned + shared since 2026-06-17, ADR-0026). Needs only 2 trust-path config changes (`role-push-guard` author tier + author allow-list) **staged in `docs/patches/0017-writer-push-autonomy.md`** + human `ALLOW_TRUST_CHANGE` merge. No `gh auth login`. |
