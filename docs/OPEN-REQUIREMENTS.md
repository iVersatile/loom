...
| **0017** Writer remote-trust | Writer pushes branches; merge-to-main gated | 🟢 **Decision 1 SHIPPED 2026-06-24 (#281)** — `loom-author` push-to-branch tier live in `role-push-guard`; merge gate intact | **FR-GUARD-PUSH** (shipped #281) | **DONE** (config gate, not credential — gh PAT was already shared/provisioned, ADR-0026). Live in the container after a `loom build`/recreate (materializes the hook). Remaining 0017 surface: the tiered-merge *amendment* (adv-068) is separate/optional. |
