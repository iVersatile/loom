# Prepared patch — drain-inbox.sh: THIN orchestrator over resurface-decide (ADR-0020 R6)

**Trust path** (`.claude/hooks/**`) → **human-applied.** *Revised 2026-06-14:*
the per-item PARK / re-surface / supersede / escalate DECISION now lives in
`config/hooks/resurface-decide` (the Writer's adv-054 slice, PR #138) — this hook
is the **thin orchestrator** that keeps the safety the decision must NOT own:
the orphan-guard (serves must match a queue row), the design-stub guard, the
drain budget, the TAKEN flip, and the block emit.

**Apply together with `0021-protect-config-hooks.md`** (which guards
`resurface-decide` from agent tampering) and only after PR #138's guard tests
are green. Order: 0021 (protect) → #138 merged → this (the drain that calls it).

## How to apply
```sh
# replace .claude/hooks/drain-inbox.sh with the content below, then:
ALLOW_TRUST_CHANGE=1 git commit -m "feat: drain — thin orchestrator over resurface-decide (ADR-0020 R6, human-applied)"
```

## What changed vs the current hook
- The predicate/park/supersede/age DECISION is delegated to the guarded
  `config/hooks/resurface-decide` (one call per drain) — no predicate logic here.
- Eligible = an item `resurface-decide` marks **DELIVER** (QUEUED) or **RESURFACE**
  (parked dep cleared). The drain still applies the orphan-guard + kind-guard to
  the chosen item (those are the drain's safety, not the decision's).
- Over-age **ESCALATE** items are surfaced in the report (one bounded escalation
  if nothing is deliverable).
- TAKEN flip matches QUEUED **or** PARKED (a re-surfaced pick).
- R8: one merged-PR cache refresh per drain, passed to `resurface-decide`.

## Validation
Smoke-tested end-to-end (pass 1 `resurface-decide` → pass 2 drain picker) — see
the relay notes. The decision engine itself is `resurface_test.go` (PR #138,
green) + the prototypes (`resurface-safe.sh` / `injection-test.sh`).
