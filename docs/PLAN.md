...
| guided-run findings batch (⑦ plan/build convergence disagreement +LL +regression; ⑧ doctor probe scope; ⑨ teardown unconfirmed; ①–⑥ doc/ergonomics gaps) | guided run ✓ (docs/guided-run.md results) | criterion-1 evidence → fixes; stranger-path ergonomics | loom-author | all in review — ⑦ #79 (LL-012, FR-PLAN-003); stacked on it in order: ⑧ (FR-DOCTOR-002), ⑨ (FR-TEARDOWN-003), ①–⑥ (README quickstart, runbook preconditions, plan names target); row flips done when the chain merges | fix/plan-build-convergence → fix/doctor-probe-scope → fix/teardown-confirm → docs/guided-run-ergonomics |
