...
| T10 non-root user / parameterized $HOME | — | hardening; one of three full-auto re-eval gates | loom-author | in review — PR 1 #122 + PR 2 #130 MERGED 2026-06-13 (design, clause, schema/merge/validate, ContainerSpec.User/Home plumbing, ADR-0019, FR-SCHEMA-009). Writer caught an ADR-0019 contradiction (decisions 2 vs 4) building PR 3 → advisor ruled MODEL A, decision 2 amended (#131): container runs as root, entry verbs `exec -u <user>` BY NAME. PR 3 MERGED (#132): run-as-user engine behavior — collision-tolerant useradd, home retarget to /home/<user>, scoped chown (prunes ro creds bind), entry-verb `-u`, FR-EXEC-004 + FR-BUILD-015 (gate + fr-verify green; docker path integration-validated on CI). PR 4 un-deferred 2026-06-15 (slice-4 spawner precondition, ADR-0022 S3). **PR4 PART 1 of 3 in review (adv-061): engine role marker** — `ContainerSpec.Role` ← `LOOM_SESSION_ROLE` (overlay/build value, NOT a playbook key; marker-safe charset), provision writes ROOT-OWNED single-line `/var/lib/loom/role` (`writeRoleMarker`/`roleMarkerScript`, read-not-forge), doctor `container:user` claim (Model A: root runtime; non-root user = entry-verb exec identity); FR-BUILD-016; tests TestValidRole/TestRoleMarkerScript/TestBuildPopulatesRole/TestDoctorContainerUser; `roleMarkerWired` unchanged. **Behavior unchanged while root** (empty role ⇒ no marker) — safe to merge anytime. Gate + fr-verify green; awaiting advisor review + git-controller push+PR. Part 2 (HUMAN, `ALLOW_TRUST_CHANGE`): the `drain-inbox.sh:23-24` guard swap reading the marker (`docs/patches/0022-drain-guard-role-marker.md`, re-diff first) AFTER this merges + recreate; Part 3 = the loom-dev non-root flip (`user:`, deferred topology call). Design point flagged for review: `Role` source = build env (alternatives: overlay file / project-name derive) | #122, #130, #132 |
