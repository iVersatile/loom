...
| T16 engine work (`harness:` schema + materialize handlers) | ADR-0015 ✓, T7 fix ✓, `harness:` clause accepted ✓ (#53) | T12 criterion 4 | loom-author | in review — PR 1 #86 + PR 2 #87 merged 2026-06-12; PR 3 in review — PR #107 (gitconfig dotfile ref + base noreply identity + doctor host:gitconfig) | #86, #87, #107 |
...
| Phase-1 review fix batch (item 024) follow-ups: build observability (F-a ensured/written summary split, F-b home.sync audit entry, F-c diag-log append; F-d lock refresh = host digest verification first) | #92 merged ✓ | live-build e2e findings (.scratch/live-build-experiment.md, R1 audit-delta family) | loom-author | in review — branch fix/r1-build-observability (stacked on fix/t4-path-single-owner); F-d held for host | — |
| guard-bash segment-aware evaluation (specimen: --force + main matched one regex across unrelated chain segments) | T29 design red-teamed + decided ✓ (conditional pass 2026-06-12) | fewer false blocks AND false prompts — deny floor match set must never shrink | loom-author | in review — impl branch feat/046-t29-segment-guard: amendments 1–5 landed (two-pass fail-closed, both patterns re-anchored w/ was-false-blocking regressions, class-action wording fixed, broad indirection taint, ONE shared splitter + cross-segment trace); T29 thread flipped ✅; allow-compound adoption = human-applied diff (trust path, envelope note) | #112 |
...
| phase-close review gate transcription (TEAM.md clause + rubric doc + RULES pointer [frozen — human admin-merge]) | — | human-decided P7 2026-06-11; gate already executed for Phase 1 (docs/reviews/phase-1-review.md) | loom-author | in review — PR #114 (TEAM.md clause + docs/review-gate.md rubric; frozen — human merge = acceptance) | #114 |
| work-selection policy transcription (cascade + principles p1–p5 + dormant mechanics; TEAM.md coordinator section, /coordinate + /replan SKILL.md) | — | human-blessed 018 2026-06-11; coordinator runs already apply it | loom-author | in review — PR #108 [TEAM.md frozen — human merge = acceptance]; /coordinate SKILL.md diff prepared not applied (classifier denial, docs/patches/023-coordinate-skill.md — human applies) | #108 |
...
| 036 harness-state durability: playbook materializes trust/opt-in flags at build ("2-plus", human-decided 2026-06-12; `.claude.json` sibling file on overlay dies on recreate — BOTH seats, advisor-verified; declare-or-rederive per devcontainer/Codespaces precedent, ADR-0014/0015 aligned) | — | Writer durable opt-in; autopilot survives recreate; prerequisite for ADR-0017 target state | loom-author | in review — PR #113 (harness.<agent>.trust -> $HOME/.<agent>.json; ADR-0018 + SPEC#harness clause ride the PR — human merge = acceptance; FR-BUILD-014) | #113 |
| settings-files write protection (029.B): extend never-auto floor + protect-paths to `.claude/settings*.json` + `.claude/hooks/**` (deny Edit/Write + pre-commit guard) | — | T28/C1-family: trust config can't be agent-rewritten; closes the in-container judgment-only gap | loom-author (harness-deny half: human) | in review — branch fix/040-trust-config-protect: protect-paths trust class + ALLOW_TRUST_CHANGE audited override (no cross-unlock with ALLOW_SPEC_CHANGE), guard suite + FR-GUARD-TRUST-CONFIG, gate scrub, TEAM never-auto floor line. REMAINDER human-only (029.C): apply the proposed Edit/Write deny diff to `.claude/settings.json` (diff in handoff/PR) — row stays open until both halves land | — |
| git-discipline transcription (028: end-on-main, currency contract, hand-off per-branch, resolver-as-tool script; 2 specimens) [TEAM clause frozen — human admin-merge] | — | shared-tree git discipline mechanized; human-blessed 2026-06-12 | loom-author | in review — branch docs/041-git-discipline-transcription: TEAM.md clause (4 rules + 2 specimens), /coordinate read-mode currency step, scripts/resolve-plan-union.go (seed transcribed python→Go — loom-dev ships no python; gate-covered tests); human admin-merge = acceptance | — |
| Writer SessionStart checkpoint hook (user-level /root/.claude, volume-backed; T28-A triad: own HALT sentinel + data-framing + 60-line cap) | — | cold-start continuity = mechanism on Writer seat (loss class: docs/e2e/cold-start-continuity.md) | loom-author | in review — branch feat/042-writer-checkpoint-hook: declared via the playbook (config/hooks/checkpoint-inject + SessionStart registration in declared settings + harness.claude.hooks) so build materializes it to /root/.claude/* — volume-backed AND declare-or-rederive (rides FR-BUILD-009; 039 precedent); T28-A triad tested (guard suite, 5 tests); reaches the live seat at next build/home-sync | — |
...
| T10 non-root user / parameterized $HOME | — | hardening; one of three full-auto re-eval gates | loom-author | design in review — envelope 045 (FIRST DRAIN-INJECTED WORK, e2e proof): T10 thread 🟢 full design (user: scalar, provision-root/run-as-user split, role marker replacing uid check, doctor claim, migration) + PR 1 on branch feat/045-t10-design-first-pr (containerHome single-owner fix + grep guard); PR 2 spec [human authorizes] / PR 3 engine / PR 4 role-marker+doctor filed as draft for triage; advisor red-team asks in thread | — |
