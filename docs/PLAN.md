...
| context economy + intake lane + /coordinate (TEAM clause, fyi/draft kinds, drain guards + tests, two-mode skill, hat authority, standup rendering) | — | cross-role context without relay or over-minting; coordinator hat mechanized | loom-author | queued — decided/blessed 2026-06-11, envelope item 012; **human-prioritized to top 2026-06-11** (T16 hook design consumes its deferred-boards decision; spec-map waits) | — |
...
| T9 shell impl (TTY sugar over exec; session-open audit) | exec merged ✓ (#43) | human topologies entry | loom-author | in review — FR-SHELL-001 (one exec path + TTY option; session-open audit, no command capture); stacked on the guided-run chain | feat/t9-shell |
