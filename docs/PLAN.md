...
| T10 non-root user / parameterized $HOME | — | hardening; one of three full-auto re-eval gates | loom-author | in review — PR 1 #122 + PR 2 #130 MERGED 2026-06-13 (design, clause, schema/merge/validate, ContainerSpec.User/Home plumbing, ADR-0019, FR-SCHEMA-009). Writer caught an ADR-0019 contradiction (decisions 2 vs 4) building PR 3 → advisor ruled MODEL A, decision 2 amended (#131): container runs as root, entry verbs `exec -u <user>` BY NAME. PR 3 MERGED (#132): run-as-user engine behavior — collision-tolerant useradd, home retarget to /home/<user>, scoped chown (prunes ro creds bind), entry-verb `-u`, FR-EXEC-004 + FR-BUILD-015 (gate + fr-verify green; docker path integration-validated on CI). PR 4 un-deferred 2026-06-15 (slice-4 spawner precondition, ADR-0022 S3). **PR4 PART 1 MERGED #154 (adv-061): engine role marker** — but it sourced the role from ambient `LOOM_SESSION_ROLE`, no-op'd silently on empty, and wrote only behind the convergence early-return, so a normal `loom build` never produced it and a verify was "passed" by a hand-run `docker exec` = drift (**LL-014**). **PART 1 REDO built (adv-062), 2 stacked branches awaiting advisor push+PR (SPEC first):** (a) `feat/0019-pr4-redo-role-field` — DECLARATIVE `role:` playbook field (SPEC-playbook `#role` + ADR-0019 §5 amendment [frozen, ALLOW_SPEC_CHANGE human-authorized, merge = acceptance] + schema/merge/validate + FR-SCHEMA-010 + `loom.yml role: loom-author`); (b) `feat/0019-pr4-redo-role-marker` — engine: `build` sources `pb.Role` (env demoted to override/test-seam), **fail-loud** `roleMarkerPlan` (non-root `user:` + empty role = HARD ERROR; root + empty = visible warning, no marker), **convergence dimension** `needsRoleMarker`/`readRoleMarker` folded into the Ensure early-return (missing/stale marker self-heals on a plain build, mirrors the home sentinel), `BuildResult.Warnings`, FR-BUILD-016 restated. Gate + fr-verify green; docker read/write integration-validated. `drain-inbox.sh` UNTOUCHED. Part 2 (HUMAN, `ALLOW_TRUST_CHANGE`): the `drain-inbox.sh:23-24` guard swap reading the marker (`docs/patches/0022-drain-guard-role-marker.md`, re-diff first) AFTER these merge + recreate; Part 3 = the loom-dev non-root flip (`user:`, deferred topology call) | #122, #130, #132, #154 |
