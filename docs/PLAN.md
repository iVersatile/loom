...
| **author autopull trial (Shape 1, 3-day)** — exercise the now-complete substrate: `AUTOPILOT` on + `LOOM_AUTOPULL_CLASSES=exec` + 2–3 deliberately `[class:exec]`-tagged rows; the author self-selects → drains → works, output = branches/git-tasks. **CANNOT reach main** (advisor-only push) ⇒ bounded blast radius, human stays on merge gate | substrate CONCLUDED (#220); self-wake VERIFIED 2026-06-19 | validates the autonomy substrate in practice; high-learning, low-risk | scaffold = loom-advisor (tag rows + trial doc); **flip-on = HUMAN** (autonomy gate) | candidate — **NEXT (lean)**. Exit: keep iff zero S1+S2; revert (`AUTOPILOT` off / HALT) pre-authorized. Shape 2 (unattended, +cold-floor nudge) graduates after a clean Shape 1 | — |
| **slice-5 HOST hardening (make the substrate load-bearing)** — host-own *append-only* external-truth files (merged-refs/ledger/tick) + relocate substrate scripts under `config/hooks/` (protect-paths) + flip `LOOM_EXTERNAL_TRUTH_STRICT=1` | slice-5 author-half ✓ #220 (consumer STRICT check ready) | tamper-resistance — trust the loop at scale; ⚠ ledger must be APPEND-ONLY not read-only (spawn-guard appends grants) | **HUMAN (host/trust)** | candidate — pairs with the trial | #220 |
