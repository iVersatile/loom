...
| guard-bash segment-aware evaluation (specimen: --force + main matched one regex across unrelated chain segments) | T29 design red-teamed + decided | fewer false blocks AND false prompts — deny floor match set must never shrink | loom-author | design in review — T29 stub (branch docs/thread-guard-segments): two-pass fail-closed + indirection taint; naive per-segment split named as a WEAKENING; advisor red-team before code | — |
