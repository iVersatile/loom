...
| T10 non-root user / parameterized $HOME | — | hardening; one of three full-auto re-eval gates | loom-author | in review — design + PR 1 merged 2026-06-12 (#122); advisor red-team PASS-w/-amendments 2026-06-13 (#126, clause amended + human-reauthorized). PR 2 (THIS): SPEC `user:` clause + schema/merge/validate + ContainerSpec.User/Home plumbing + ADR-0019 + FR-SCHEMA-009 (gate green; human merge = C3 acceptance). NEXT: PR 3 engine behavior (`docker run --user`, useradd, scoped chown) / PR 4 role-marker [trust path, human diff] + doctor `container:user` = draft 047 | #122 |
