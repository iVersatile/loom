...
| T16 engine work (`harness:` schema + materialize handlers) | ADR-0015 ✓, T7 fix ✓, `harness:` clause accepted ✓ (#53) | T12 criterion 4 | loom-author | PR 1 + PR 2 in review (PR 1: schema+merge+validate+materialize, FR-BUILD-009/FR-SCHEMA-008; PR 2: base harness: declared — settings owns hook registrations + deny floor, guard-bash to ~/.claude/hooks, verify flips, dogfood test; fixed PR 1's merged-tier validation defect); PR 3 = gitconfig+doctor | feat/t16-harness-schema → feat/t16-base-wireup |
...
| context economy + intake lane + /coordinate (TEAM clause, fyi/draft kinds, drain guards + tests, two-mode skill, hat authority, standup rendering) | — | cross-role context without relay or over-minting; coordinator hat mechanized | loom-author | in review — T25 stub + full package (inbox item 012) | feat/coordinate |
...
| guided-run findings batch (⑦ plan/build convergence disagreement +LL +regression; ⑧ doctor probe scope; ⑨ teardown unconfirmed; ①–⑥ doc/ergonomics gaps) | guided run ✓ (docs/guided-run.md results) | criterion-1 evidence → fixes; stranger-path ergonomics | loom-author | in progress — ⑦ in review (LL-012, FR-PLAN-003); ⑧⑨①–⑥ follow as split PRs | fix/plan-build-convergence |
| /specmap project skill (regenerates docs/spec-map.md; writes via PR — inbox item 011; row may dup docs/specmap-row) | spec-map v1 ✓ (#67) | spec→FR→thread shape on demand | loom-author | in review | feat/specmap-skill |
