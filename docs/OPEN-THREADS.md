...
Pointers: ADR-0022 cold-floor · `scripts/cold-floor-cron` (#176) · advisor /loop git-controller sidecar (proto-supervisor) · T27 (wake/observability) · **T35** (the cron host-glue #216/#217 = temp measure; 3 concerns this box would resolve) · memory host-action-delta-op-triage · couples to T18 (who runs/declares the supervising layer).
...
## T35 — cron host-glue (manager + actuators) is a TEMP measure — revisit (host-tool dependency, send-key injection security, against the self-contained-container loom-way)   🟡 open — PARKED (documented 2026-06-19; NOT worked now; subsumed by T32)
Surfaced 2026-06-19 wiring the daily `/coordinate` standup + the cold-floor on a Mac host. The toolchain — actuators `scripts/coordinate-cron` (#216) + `scripts/cold-floor-cron`, installed by the host-cron MANAGER `scripts/loom-cron` (#217) — WORKS and is the pragmatic path TODAY (host crontab → `docker exec loom-dev` → actuator → inject). It is **explicitly a temporary measure.** Three concerns to revisit before it is load-bearing/permanent:
1. **Host-tool dependency.** The inject hardcodes `tmux send-keys`, creating a host dependency on tmux (and a tmux-hosted session) that will NOT hold across hosts/topologies (windows-dev, ai-user, headless). The actuator's wake/inject mechanism must not assume a specific host multiplexer.
2. **Injection security.** Driving a live, trusted agent session via `tmux send-keys` is an out-of-band **keystroke injection** — a weaker channel than the guarded inbox/hook path: no role-guard, no HALT-on-the-input, no audit of what is typed. Operator-env trust covers the *origin*, but the channel itself bypasses the harness input guards. Needs a real threat model before it is relied on.
3. **Against the self-contained / self-evolving dev-container principle (not the loom-way).** Host-side cron + docker/tmux reach re-introduces host state the loom-way is meant to dissolve — exactly the "host ops are a smell" T32 names (tools live in the container by design). The durable answer is an always-on **supervising box** (T32) owning the schedule + actuator-reach from inside the loom topology, not the host.
**Disposition:** PARKED — documented, NOT worked now (focus = current workstream / T34 Slice D). This is a concrete sub-facet of **T32** ("the cold-floor cron is the one piece that still needs a host crontab purely for its always-on + wake-reach properties") sharpened to name the *inject mechanism* (tmux-send-keys) and its security. Revisit when T32 is taken up; until then the #216/#217 host-glue stands as the pragmatic interim.
Pointers: **T32** (supervising-box — home of the resolution) · T34 (advisor-in-loom — the self-contained-container direction) · #216 (coordinate-cron) · #217 (loom-cron manager) · `scripts/{coordinate-cron,cold-floor-cron,loom-cron}` · ADR-0022 cold-floor · `docs/cold-floor-trial.md` §"why a host cron at all".
