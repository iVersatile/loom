...
- **Sequence:** finish **Slice A** → **thin-slice** (an advisor session in loom-dev pushes a branch + opens a PR — proves A+B+C) → **Slice E** (declare advisor in the playbook, full-parity git-controller/confer/assign cycle, retire devenv ≤2026-06-24) → **Slice D** (in-container spawn + reaper + spawn-ledger host/append-only, built NATIVELY in-loom). Rationale: D's spawn-allow is cleanest built after advisor-in-loom (avoids a throwaway `loom exec` spawn path).
- **Slice status:** A (multi-role provisioning) IN PROGRESS · B (gh-cred volume #188 + advisor allowlist; **fine-grained PAT provisioned in loom-dev, human 2026-06-18**) ✓ · C (own-worktree + edit-guard, ADR-0023 #186) ✓ · the role-deny guards are **LIVE at HEAD** (role-push-guard #189/#192, spawn-guard #194, FR-GUARD-E2E #197, HARNESS caveat #198) · E + D pending.
