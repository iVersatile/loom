...
## T15 — the working auth path is human-only; AI-first auth needed   🟢 SLICE 1 SHIPPED 2026-06-22 — ADR-0027 = per-project credential CONVENTION (Accepted #243); **slice 1 built + human-merged (#252)**: `harness.<agent>.credential {method,helper,env}` (any-tier merge like `trust`; enum-validated; unwired methods fail-CLOSED) + both claude adapters — **M1 volume-token** (subscription; exec-time `-e` injection onto the agent seat, NEVER `docker run -e`, no ADR-0014 leak; fail-closed) + **M2 apiKeyHelper** (metered; targeted settings.json key-set) + doctor `host:credential-slug` (slug-uniqueness invariant) + `FR-CRED-001..004`. apiKeyHelper TUI+headless probe-confirmed (`FAKE_KEY=1`, zero cost). **NEXT (slice 2+):** the **`oauth-file` adapter** (the Gemini ADC spike's verdict, below — NOT the resolver) + `serviceVolume` generalization + multi-M1-agent + the cross-project-token-reuse & billing-identity doctor checks + the `env:`-name charset NIT. The universal **RESOLVER stays DEFERRED** (the spike found Gemini does not justify it). Real token/key provisioning stays a human trust act.

**Gemini ADC spike (2026-06-22, loom-author; `.scratch/spikes/t15-gemini-adc-resolver.md` + probe `.scratch/spikes/probe-gemini-adc.sh`).** The deferred-resolver gate (ADR-0027 §4: "the Gemini ADC spike … turns the resolver from premature into extractable") has been run as an analysis spike. **VERDICT: STILL DEFER the universal resolver — Gemini does NOT justify it.** Gemini-cli's three auth modes collapse into two ALREADY-named adapters: (a) `GEMINI_API_KEY`/`GOOGLE_API_KEY` = **S1 / `env`** (the M2 family, already wired-shape); (b) OAuth login (`~/.gemini/oauth_creds.json`) + (c2) gcloud ADC (`~/.config/gcloud/application_default_credentials.json`) = **S2 / `oauth-file`**; (c1) Vertex SA-JSON (`GOOGLE_APPLICATION_CREDENTIALS`) = **`volume-token`-with-file** (`:ro`, no refresh-write). In EVERY class refresh is the CONSUMER's job, never loom's — so the resolver §4 deferred has **no work the per-adapter materialize dispatch isn't already doing**; the rule-of-three's third example is the one MOST like gh, not an orthogonal new axis (the exact ADR-0027 rev4 trap). **What Gemini DOES earn = a small, concrete `oauth-file` adapter:** a **writable** (`:rw`) per-project volume at the consumer's fixed home path (`$HOME/.gemini` / `$HOME/.config/gcloud`) the CLI mutates on token-refresh — the ONE genuinely-new engine fact (M1's volume is `:ro`; `oauth-file` must be `:rw`). It is the gh `<container>-gh` store-volume pattern generalized one notch (drop the helper line, add an optional path-env), and it finally motivates the deferred `serviceVolume` refactor (`container.go:374` hardcoded `{agentHomeVolume, ghConfigVolume}` → derived set). loom places nothing and refreshes nothing (human does the one browser login; CLI refreshes in place); fail-closed check = "volume present + non-empty creds file", never reads the token. **Load-bearing fact rests on [I]:** that the REAL gemini refresh writes back to the volume (→ the `:rw` requirement) — proven loom-side at zero cost by the probe's **`FAKE=1` mode** (per-project rw-volume persistence + writable-refresh + `:ro`-rejects-write), confirmed gemini-side only by the human's `LIVE_LOGIN=1` run. **NEXT:** human runs `FAKE=1 bash .scratch/spikes/probe-gemini-adc.sh` (free, zero login/network) to green-light the adapter; then build `oauth-file` as slice 2 (secrets feature → human merge). T20 egress floor for Gemini: `generativelanguage.googleapis.com`, `oauth2.googleapis.com` (refresh dies without it), `accounts.google.com` (login bootstrap), `cloudcode-pa.googleapis.com` [I/LIVE], `aiplatform.googleapis.com`+regional — none wired today; the LIVE probe's `EGRESS_TRACE=1` captures the minimal set.
...
## T20 — container-level egress restriction: the allowlist's arbitrary-code escape hatch   ✅ COMPLETE 2026-06-22 — S1 ✅ (#246) · ADR-0028 ACCEPTED (#247) · S2a ✅ (#248, `networking:` field + `egress: none`) · **ADR-0028 Amendment 1 (#250)** — the allowlist mechanism = an **SNI/CONNECT proxy sidecar** (NOT the IP-network/per-IP-firewall the original plan named; the 3-agent spike + the #249 real-docker proof showed the IP path is Linux-only via DOCKER-USER + fights the ~80%-rotating-CDN allowlist) · **S2b ✅ (#251)** — `egress: allowlist` = a loom-managed proxy sidecar as the container's SOLE route (an `--internal` docker net; the route is the fence, not the HTTP(S)_PROXY env), shipped as a hidden `loom __egress-proxy` subcommand (Delivery A: the loom binary cp'd into the sidecar — the first `go run`-in-sidecar attempt broke on non-Go bases), with a hardcoded load-bearing floor (model API + OAuth + telemetry + github) union'd in so a forgotten host can't brick the agent. Adversarial review: fail-closed + no-bypass HOLD. **Unblocked the ADR-0027 credential build (now slice-1 shipped, #252).** Deferred LOW/NIT follow-ups: tighten doctor to assert the net set is exactly {intNet} · proxy dial/idle timeout · realize observe→enforce (only the logging surface shipped) · note allowlisted hosts reachable on any port (ADR = host-level by design) · the LOW-2 strict-playbook-decoding hardening (bears on T28).
...
## T38 — Role-model generalization: identity + layered boundaries   🟡 open (full draft: `docs/threads/T38-role-model-generalization.md`)

Triggered 2026-06-24 when a human scenario (project-ABC: planner/dev/supervisor + DB-owner/data-manager, staging→PROD pipeline) broke the two-role assumption D3 rested on. **Principle (human):** a loom role is a generic, forge-proof *identity*; loom prescribes as little as possible about what it can *do*; boundaries are applied by the layer that owns each one, keying off the identity. **Bright line:** "generic" = an identity later layers bind boundaries to, NOT loom enforcing arbitrary declared powers (that = a weaker, bypassable copy of downstream authz — the DB lesson; *guardrails are mechanism, not trust*). **Decisions (human):** de-hardcode `loom-author`/`loom-advisor` → a thin reviewed `role→git-tier` map; a generalized role = an identity → a *policy across domains*; roles stay a union per container (ADR-0021). **Key model move — capability taxonomy re-derived into 3 levels by loom's relationship to the object** (the first flat list mixed levels — `shell/exec` is the universal lever, not a sibling of `git`): **L1 loom-owned substrate** (harness/trust/lifecycle/agent-fabric — loom enforces, least delegable) · **L2 execution surface** (`shell`/`exec` — the meta-capability, `guard-bash` gates within it) · **L3 external resources** (forge/network/DB/cloud — loom *gates path + routes scoped credential*, the **downstream system enforces**; the credential bundle is the seam DB-owner/data-manager/cloud attach to). **Zoom-out (what is loom):** a *deepening* of loom's existing identity (the AI-agent authority layer it already is), NOT a new product — **iff the bright line holds** (loom owns L1–L2 + gates/routes L3; downstream enforces L3 domains). Cross the line (loom enforces downstream domains) → a general IAM platform = likely a charter non-goal. **Gates D3** (PARKED + human-gated). Open Qs: where the `role→tier`/`role→cred` map lives + unforgeability · is the push/merge guard defense-in-depth over forge branch-protection · selective/branch-scoped merge = forge or loom · union-vs-single validation shape. Status: 🟡 confer queued to author (`q-1782327925`/`q-1782328509`); promotes to an ADR only on explicit human go-ahead.
Pointers: `docs/threads/T38-role-model-generalization.md` (full draft) · ADR-0021 (role resolution; trust-role union) · ADR-0017 (writer push tier) · ADR-0014/0026/0027 (credential adapters — the L3 routing seam) · ADR-0019 §5 + `SPEC-playbook#role` (the D3 drift, parked) · `config/hooks/role-push-guard` (today's hardcoded 2-role gate) · D3 row (PARKED).
